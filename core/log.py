...
def _log(config, step_count, log_data, model, replay_buffer, lr, shared_storage, summary_writer, vis_result):
    ...
    if test_dict is not None:
        ...
        median_score = np.mean(test_dict['median_score'])
        test_msg = '#{:<10} Test Mean Score of {}: {:<10} (max: {:<10}, min:{:<10}, median:{:<10}, std: {:<10})' \
                   ''.format(test_counter, config.env_name, mean_score, max_score, min_score, median_score, std_score)
